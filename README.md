...
# Ishchi funksiyalari (o'zgarmagan, faqat UTC+5 qo'shildi)
async def clock_in(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    now_uz = datetime.utcnow() + timedelta(hours=5)  # UTC+5
    today = now_uz.strftime("%Y-%m-%d")
    now = now_uz.strftime("%H:%M:%S")
...
async def clock_out(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not row:
        ...
        end = datetime.utcnow() + timedelta(hours=5)  # UTC+5
        if end < start:
